...
The `clean` targets remove downloaded data directories and created files. This is useful for reproducing a fresh download, but the README should clearly warn users that the command deletes local data.

## README Evaluation

The repository documents the selected organism, the required tools, and the expected FASTA and GFF outputs. The Week 03 Makefile also reports genome size, sequence count, and annotation feature count, which makes the result easier to verify.
...
One clarity issue is that the Week 02 `clean` recipe removes directories after separately naming files for removal. Since directory deletion already removes those files, the redundant file-removal line can be simplified. The README should also show the exact command used to count annotation features.
...
This links the review directly to the reproducible workflow used to generate the reported values.
